...
| April 2026 | System tab redesigned: `SystemStatusSection` removed (hardcoded, non-actionable scanner/LinkedIn/digest cards). Replaced with `SystemIntegrationCards` — two compact side-by-side hero cards (Slack Digest, CRM Integration) showing plan-gated upgrade prompts for locked plans and live connection status for unlocked plans. Slack and CRM locked gates now return null (overview card handles upgrade UX). Fixed "6 AM" timing bug in `buildSettingsOpening()` → "~8 AM Pacific (3 PM UTC)". |
